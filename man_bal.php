...
if(isset($_POST['submit'])){
            ...
            if($action=='add'){
                $sql="INSERT INTO `transaction`(`mode`, `type`, `to_from`, `amount`) VALUES ('$mode','IN','ADMIN','$amt');";
                ...
                if($result){
                    echo "<script>
                        alert(\"Transaction Successful !!\");
                         location.replace(\"man_bal.php\") ;</script>";
                }
            }
            else{
                if($action=='widr'){
                $sql="INSERT INTO `transaction`(`mode`, `type`, `to_from`, `amount`) VALUES ('$mode','OUT','ADMIN','$amt');";
                $resul=mysqli_query($conn,$sql);
                if($amt<=$bal){
                $newbal=$bal-$amt;
                $sql2= "DELETE from  `balance`";
            ...
            echo "<script>
                        alert(\" Cannot Withdraw AMOUNT ENTERED > AVAILABLE BALANCE  !!\");
                         </script>";}
            }
...
<a href="admin_settings.php"><i class="fas fa-cog"></i><span>Settings</span></a>
    ...
    <div class="content">
        <h3><a href="admin_sales.php"><i class="fas fa-arrow-left"></i></a> Add / Withdraw Funds</h3>
        <div class="row">
            <div class="col-11">
                ...
                <form method="POST">
                    ...
                    <input type="radio" name="action" value="add" required> Add Funds
                    &nbsp&nbsp&nbsp&nbsp                   
                             
                    <input type="radio" name="action" value="widr" required>  Withdraw Funds                  
                </td>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="submit" name="submit" value="Confirm" id="subbtn">
                            </td>
                        </tr>
                    </table>
                </form>
